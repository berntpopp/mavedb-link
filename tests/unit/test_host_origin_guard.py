...
from mavedb_link.config import ServerSettings, settings
from mavedb_link.server_manager import create_unified_app
...
def client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    monkeypatch.setattr(
        settings,
        "allowed_hosts",
        ["localhost", "127.0.0.1", "::1", "mavedb-link.genefoundry.org"],
    )
...
def test_fastmcp_344_strict_guard_is_installed(client: TestClient) -> None:
    ...
    response = client.get("/mcp", headers={"Host": "mavedb-link.genefoundry.org"})
...
def test_absent_and_configured_origins_are_allowed(client: TestClient) -> None:
    no_origin = client.get("/mcp", headers={"Host": "mavedb-link.genefoundry.org"})
    configured_origin = client.get(
        "/mcp",
        headers={
            "Host": "mavedb-link.genefoundry.org",
...
def test_untrusted_origin_is_rejected_on_every_route(client: TestClient, path: str) -> None:
    response = client.get(
        path,
        headers={"Host": "mavedb-link.genefoundry.org", "Origin": "https://evil.example"},
    )
...
def test_json_environment_allowlists_are_parsed(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(
        "MAVEDB_LINK_ALLOWED_HOSTS",
        '["localhost","mavedb-link.genefoundry.org"]',
    )
    monkeypatch.setenv("MAVEDB_LINK_ALLOWED_ORIGINS", '["https://genefoundry.org"]')
    configured = ServerSettings(_env_file=None)
    assert configured.allowed_hosts == ["localhost", "mavedb-link.genefoundry.org"]
