...
format: ## Format Python code
	uv run ruff format mavedb_link tests server.py mcp_server.py scripts

format-check: ## Check formatting without writing
	uv run ruff format --check mavedb_link tests server.py mcp_server.py scripts

lint: ## Lint Python code
	uv run ruff check mavedb_link tests server.py mcp_server.py scripts

lint-ci: ## Lint with GitHub-Actions output
	uv run ruff check mavedb_link tests server.py mcp_server.py scripts --output-format=github

lint-fix: ## Lint and apply safe fixes
	uv run ruff check mavedb_link tests server.py mcp_server.py scripts --fix

lint-loc: ## Enforce per-file line budget
	uv run python scripts/check_file_size.py

typecheck: ## Type check package
	uv run mypy mavedb_link server.py mcp_server.py
...
test-cov: ## Run tests with coverage
	uv run pytest tests -m "not integration" --cov=mavedb_link --cov-report=term-missing --cov-report=html
...
docker-url: ## Print the MCP URL (host port the container is published on)
	@hostport=$$($(COMPOSE) port mavedb-link 8000 2>/dev/null); \
	port=$${hostport##*:}; \
	if [ -n "$$port" ]; then \
	  echo "mavedb-link MCP: http://127.0.0.1:$$port/mcp  (health: http://127.0.0.1:$$port/health)"; \
	  echo "Claude Code: claude mcp add --transport http mavedb-link --scope user http://127.0.0.1:$$port/mcp"; \
	else \
	  echo "mavedb-link container is not running. Start it with: make docker-up"; \
	fi

info: ## Show project information
	@echo "Project: mavedb-link"
