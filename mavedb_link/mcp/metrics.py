...
the build + upstream-reachability provenance. The latency window is bounded so
memory stays constant, and the collector is process-local (reset on restart, and
in tests).
"""
...
#: Rolling latency window. Percentiles are over the most recent calls; counters
#: (requests/errors) are cumulative for the process lifetime.
_MAX_SAMPLES = 1024

#: Minimum request count before ``error_rate`` is reported (below this the ratio
#: reads as noise rather than signal). Raw counts are always reported.
