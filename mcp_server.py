...
def main() -> None:
    """Run the mavedb-link MCP server on the stdio transport."""
    # Configure environment BEFORE importing anything that may print to stdout.
    os.environ.setdefault("MAVEDB_LINK_TRANSPORT", "stdio")
    ...
    try:
        ...
        from mavedb_link.server_manager import UnifiedServerManager
    except Exception as exc:
        print(f"ERROR: mavedb_link import failed: {exc}", file=sys.stderr)
        sys.exit(1)
